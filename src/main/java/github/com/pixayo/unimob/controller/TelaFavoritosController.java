...
public class TelaFavoritosController extends BaseController {

    @FXML private VBox emptyState;
    @FXML private ScrollPane scrollFavoritos;

    @FXML private HBox card006;
    @FXML private HBox card082;
    @FXML private HBox card3213;
    ...
    public void atualizarTela() {
        ...
        if (card006 != null) { card006.setVisible(false); card006.setManaged(false); }
        if (card082 != null) { card082.setVisible(false); card082.setManaged(false); }
        if (card3213 != null) { card3213.setVisible(false); card3213.setManaged(false); }

        for (LinhaService.LinhaOnibus linha : LinhaService.getLinhasFavoritas()) {
            temFavorito = true;
            if (linha.getNumero().equals("0.006") && card006 != null) { card006.setVisible(true); card006.setManaged(true); }
            if (linha.getNumero().equals("0.082") && card082 != null) { card082.setVisible(true); card082.setManaged(true); }
            if (linha.getNumero().equals("3213") && card3213 != null) { card3213.setVisible(true); card3213.setManaged(true); }
        }

        if (temFavorito) {
            if (emptyState != null) { emptyState.setVisible(false); emptyState.setManaged(false); }
            if (scrollFavoritos != null) { scrollFavoritos.setVisible(true); scrollFavoritos.setManaged(true); }
        } else {
            if (emptyState != null) { emptyState.setVisible(true); emptyState.setManaged(true); }
            if (scrollFavoritos != null) { scrollFavoritos.setVisible(false); scrollFavoritos.setManaged(false); }
        }
    }

    @FXML private void removerFavorito006(ActionEvent event) { desmarcar("0.006"); }
    @FXML private void removerFavorito082(ActionEvent event) { desmarcar("0.082"); }
    @FXML private void removerFavorito3213(ActionEvent event) { desmarcar("3213"); }

    private void desmarcar(String numero) {
        for (LinhaService.LinhaOnibus l : LinhaService.getTodasAsLinhas()) {
            if (l.getNumero().equals(numero)) {
                l.setFavoritado(false);
                break;
            }
        }
        atualizarTela();
    ...
    @FXML private void abrirTelaFavoritos(ActionEvent actionEvent) { sceneManager.displayScene(SceneName.TELA_FAVORITOS); atualizarTela(); }
